...
function tasteyFooterMenu() {
  ...
  document
    .querySelector(".footer-special-section > a")
    .insertAdjacentElement("afterend", offersContainer);
}

//DOM operations
getDOMElements();

const footerAddToCartBtns = document.querySelectorAll(
...
document
  .querySelector(".view-bag")
  .addEventListener("click", () => (sessionStorage.open_cart = true));

footerAddToCartBtns.forEach(
  (btn) => (btn.onclick = (e) => handleAddMeal(e.target.dataset.id)),
);

let footerItv,
  footerInView = false,
  footerInterval = 2000;
const offerGap = remToPx(0.6);
...
offersImgsWrapper.addEventListener(
  "scroll",
  offersScrollDebouncer.debounce(arrowSetting),
);

function arrowSetting() {
  offerPrevArrow.classList.toggle(
    "disabled",
    offersImgsWrapper.scrollLeft === 0,
  );
  offerPrevArrow.title =
    offersImgsWrapper.scrollLeft === 0 ? " " : "Scroll Left";
  offerNextArrow.classList.toggle(
    "disabled",
    offersImgsWrapper.scrollLeft >
      offersImgsWrapper.scrollWidth -
        offersImgsWrapper.parentElement.getBoundingClientRect().width -
        offerGap,
  );
  offerNextArrow.title =
    offersImgsWrapper.scrollLeft >
    offersImgsWrapper.scrollWidth -
      offersImgsWrapper.parentElement.getBoundingClientRect().width -
      offerGap
...
function playOffers() {
  ...
  footerItv = setInterval(() => {
    ...
    if (
      ...
      offersImgsWrapper.scrollWidth -
        document
          .querySelectorAll(".footer-tastey-meal")[0]
...
function nextOffer() {
  const value =
    document.querySelectorAll(".footer-tastey-meal")[0].getBoundingClientRect()
      .width +
    offerGap +
    offersImgsWrapper.scrollLeft;
...
offersImgsWrapper.parentElement.addEventListener(
  "touchstart",
  () => {
    stopOffers();
...
document.addEventListener(
  "keydown",
  keyThrottler.throttle(function (e) {
    if (footerInView) {
