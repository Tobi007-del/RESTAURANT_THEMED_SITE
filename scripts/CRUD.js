...
import {
  ...
  rand,
  clamp,
  handleContentEditableEnterKeyPress,
} from "./utils.js";
...
//DOM Elements
let heartIconWrappers,
  orderReviewSectionContent,
  wishlistTogglers,
  tasteyMeals,
  tasteyMealOrders,
  mealCart,
...
function getDOMElements() {
  ...
  if (miniBagQuery) {
    (miniMealCart = document.querySelector(".mini-meal-cart")),
      ...
      (mcTasteyMealOrders = document.getElementsByClassName(
        "mini-cart-tastey-meal-order",
      )),
      (mcTasteyOrderImages = document.getElementsByClassName(
        "mini-cart-tastey-order-image",
      )),
      (mcWishlistTogglers = document.getElementsByClassName(
        "mini-cart-wishlist-toggler",
      )),
      (mcDeleteOrderBtns = document.getElementsByClassName(
        "mini-cart-delete-order",
      )),
      (mcCartOrderNumbers = document.getElementsByClassName(
...
function handleCardPointerStart(e, currentCard) {
  ...
  currentCard.addEventListener(
    "touchmove",
    (currentCard.cpi = (e) => handleCardPointerInit(e, currentCard)),
...
function handleCardPointerInit(e, currentCard) {
  ...
  currentCard.addEventListener(
    "touchmove",
    (currentCard.cpm = (e) => handleCardPointerMove(e, currentCard)),
    { passive: false },
  );
}
...
function handleCardPointerMove(e, currentCard) {
  ...
  currentCard.cpRAF = requestAnimationFrame(() => {
    ...
    currentCard.style.setProperty(
      "opacity",
      clamp(0, 1 - Math.abs(deltaX) / currentCard.offsetWidth, 1),
    );
    currentCard.dataset.pointerTicker = false;
  });
...
function handleCardPointerEnd(currentCard) {
  ...
  if (
    Math.abs(parseFloat(currentCard.dataset.pointerDeltaX)) >
    currentCard.offsetWidth * 0.4
  ) {
    const dir =
      parseFloat(currentCard.dataset.pointerDeltaX) > 0 ? "right" : "left";
    handleDeleteMeal(currentCard.dataset.id, dir);
    return;
  }
  currentCard.dataset.pointerTicker = false;
  currentCard.removeEventListener("touchmove", currentCard.cpi, {
    passive: true,
    once: true,
  });
  currentCard.removeEventListener("touchmove", currentCard.cpm, {
    passive: false,
  });
...
function getCardsQuery() {
  return (
    ...
    document.body.dataset.cart != 0 &&
    window.innerWidth >= remToPx(55) &&
    window.innerHeight >= remToPx(32.55) &&
    CSS &&
...
const liftOffset = () =>
  pxToRem(tasteyMealOrders[0].getBoundingClientRect().height) - 6;

function isCardStacked(i) {
  const currTop = Math.round(
    pxToRem(tasteyMealOrders[i].getBoundingClientRect()?.top),
  );
  const prevTop = Math.round(parseFloat(tasteyMealOrders[i].dataset?.top));
  const nCurrTop = Math.round(
    pxToRem(
      tasteyMealOrders[i].nextElementSibling?.getBoundingClientRect()?.top,
    ),
  );
  const nPrevTop = Math.round(
    parseFloat(tasteyMealOrders[i].nextElementSibling?.dataset?.top),
  );
  if (currTop > prevTop || nCurrTop >= nPrevTop + liftOffset()) return false;
  else return true;
}
...
function positionCards() {
  if (getCardsQuery() && !Tastey.isEmpty()) {
    const orsrect = orderReviewSectionContent.getBoundingClientRect(),
      ...
      gap = pxToRem(
        (checkoutSection?.getBoundingClientRect().height -
          (rect.height +
            orderNumberWrapper?.getBoundingClientRect().height +
            remToPx(0.75))) /
          tasteyMealOrders.length,
      );
    const bottom = pxToRem(
      window.innerHeight -
        (remToPx(9.1 + tasteyMealOrders.length * gap) + rect.height),
    );
    ...
    for (let i = 0; i < tasteyMealOrders.length; i++) {
      ...
      tasteyMealOrders[i].onpointerover = () => {
        setTimeout(() => {
          if (tasteyMealOrders[i]?.matches(":hover")) liftCard();
        }, 500);
      };
      tasteyMealOrders[i].onmouseleave = () =>
      ...
      for (const button of tasteyMealOrders[i].querySelectorAll("button")) {
        button.onfocus = () => {
          if (getCardsQuery())
            if (button.matches(":focus-visible")) moveToCard();
        };
      }
      tasteyMealOrders[i].removeEventListener(
        "touchstart",
        tasteyMealOrders[i].cps,
      );
      tasteyMealOrders[i].removeEventListener(
        "touchend",
        tasteyMealOrders[i].cpe,
      );
      tasteyMealOrders[i].addEventListener(
        "touchstart",
        (tasteyMealOrders[i].cps = (e) => {
      ...
      tasteyMealOrders[i].addEventListener(
        "touchend",
        (tasteyMealOrders[i].cpe = (e) =>
          handleCardPointerEnd(tasteyMealOrders[i])),
      );
      function liftCard() {
      ...
      function moveToCard() {
        let pos =
          orsrect.height -
          (orsrect.height -
            ((rect.height - remToPx(gap) + remToPx(1.25)) * (i + 1) +
              orderNumberWrapper.getBoundingClientRect().height +
              remToPx(2.5))) -
          rect.height;
        scrollContentTo(pos);
...
function adjustCards() {
  if (getCardsQuery() && !Tastey.isEmpty()) {
    let hCurrTop = Math.round(
      pxToRem(orderNumberWrapper.getBoundingClientRect()?.top),
    );
    let hPrevTop = Math.round(5.5);
    let fCurrTop = Math.round(
      pxToRem(tasteyMealOrders[0]?.getBoundingClientRect()?.top),
    );
    let fPrevTop = Math.round(parseFloat(tasteyMealOrders[0]?.dataset?.top));
    if (hCurrTop == hPrevTop && fCurrTop < fPrevTop + 1.25) {
      orderNumberWrapper.style.setProperty(
        "--sticky-scale",
        `${1 - allMeals.length / 950}`,
      );
    }
    if (hCurrTop > hPrevTop || fCurrTop >= fPrevTop + 1.25) {
    ...
    for (let i = 0; i < tasteyMealOrders.length; i++) {
      if (isCardStacked(i)) {
        tasteyMealOrders[i].style.setProperty(
          "--sticky-scale",
          `${1 - (allMeals.length * ((tasteyMealOrders.length - i) / tasteyMealOrders.length)) / 950}`,
        );
      } else {
  ...
  if (getCardsQuery()) {
    if (Tastey.ordersInTotal === 0) {
      ...
      const order = document.querySelector(
        `.tastey-meal-order[data-id="${id}"]`,
      );
      order?.classList.remove("lift");
      order?.animate(
        {
          transform: `translate(${dir === "auto" ? rand(-20, 20) : dir === "right" ? 100 : -100}%, ${pxToRem(window.innerHeight) - parseFloat(order.dataset?.top)}rem)`,
        },
        { duration: removeStall, fill: "forwards" },
      );
    }
  }
}
...
function removeExtras() {
  orderNumberWrapper.animate(
    {
      transform: `translate(${rand(-20, 20)}%, ${pxToRem(window.innerHeight) - 6}rem)`,
    },
    { duration: emptyStall },
  );
...
function removeAllCards() {
  if (getCardsQuery()) {
    ...
    allTasteyMealOrders.forEach((order, i) => {
      const stall =
        ((allTasteyMealOrders.length - i) / allTasteyMealOrders.length) * 200 +
        (emptyStall - 1000);
      order?.classList.remove("lift");
      order?.animate(
        {
          transform: `translate(${rand(-15, 15)}%, ${pxToRem(window.innerHeight) - parseFloat(order.dataset?.top)}rem)`,
        },
        { duration: stall, fill: "forwards" },
      );
    });
  }
}
...
const offset = () =>
  pxToRem(
    mcOrderReviewSection?.getBoundingClientRect().height -
      mcTasteyMealOrders[0]?.getBoundingClientRect().height,
  ) - 4;

function isMCardStacked(i) {
  const rect = mcTasteyMealOrders[i].getBoundingClientRect();
  if (rect.top > remToPx(0.775 + gap * i) + rect.height) return false;
  else return true;
}
...
function positionMiniCards() {
  if (getMiniCardsQuery() && !Tastey.isEmpty()) {
    ...
    const orsrect = mcOrderReviewSection.getBoundingClientRect(),
      rect =
        mcTasteyMealOrders[
          mcTasteyMealOrders.length - 1
        ].getBoundingClientRect(),
      bottom =
        pxToRem(orsrect.height) -
        (mcTasteyMealOrders.length * gap + pxToRem(rect.height));
    mcOrderReviewSection.style.setProperty("--mini-bottom", `${bottom}rem`);
    for (let i = 0; i < mcTasteyMealOrders.length; i++) {
      mcTasteyMealOrders[i].style.setProperty(
        "--mini-sticky-top",
        `${0.25 + i * gap}rem`,
      );
      mcTasteyMealOrders[i].onclick = () => {
        if (document.activeElement.tagName.toLowerCase() !== "button")
          moveToCard();
      };
      ...
      mcTasteyMealOrders[i].removeEventListener(
        "touchstart",
        mcTasteyMealOrders[i].cps,
      );
      mcTasteyMealOrders[i].removeEventListener(
        "touchend",
        mcTasteyMealOrders[i].cpe,
      );
      mcTasteyMealOrders[i].addEventListener(
        "touchstart",
        (mcTasteyMealOrders[i].cps = (e) => {
      ...
      mcTasteyMealOrders[i].addEventListener(
        "touchend",
        (mcTasteyMealOrders[i].cpe = () =>
          handleCardPointerEnd(mcTasteyMealOrders[i])),
      );
      function moveToCard() {
        let pos =
          orsrect.height -
          (orsrect.height -
            (rect.height - remToPx(gap) + remToPx(0.5)) * (i + 1)) -
          rect.height;
  ...
  if (getMiniCardsQuery() && !Tastey.isEmpty()) {
    for (let i = 0; i < mcTasteyMealOrders.length; i++) {
      if (isMCardStacked(i)) {
        mcTasteyMealOrders[i].style.setProperty(
          "--mini-sticky-scale",
          `${1 - (allMeals.length * ((mcTasteyMealOrders.length - i) / mcTasteyMealOrders.length)) / 950}`,
        );
      } else {
...
function removeMiniCard(id, dir = "auto") {
  if (getMiniCardsQuery()) {
    ...
    const order = document.querySelector(
      `.mini-cart-tastey-meal-order[data-id="${id}"]`,
    );
    if (dir === "auto")
      order?.animate(
        { transform: `translate(${rand(-10, 10)}%, 15rem)` },
        { duration: removeStall, fill: "forwards" },
      );
    else
      order?.animate(
        {
          transform: `translate(${dir === "right" ? 100 : -100}%, ${rand(10, 40)}%)`,
        },
        { duration: removeStall, fill: "forwards" },
      );
  }
  ...
  if (getMiniCardsQuery()) {
    ...
    allTasteyMealOrders.forEach((order, i) => {
      const stall =
        ...
        (emptyStall - 1000);
      order?.animate(
        { transform: `translate(${rand(-15, 15)}%, 15rem)` },
        { duration: stall, fill: "forwards" },
      );
    });
  }
...
function setCartStates() {
  ...
  dataCartStates.forEach((dataCartState) => {
    if (dataCartState.classList.contains("navbar-cart")) {
      dataCartState.dataset.cart = standardize(
        Tastey.ordersInTotal,
        "use strict",
      );
      return;
    }
...
function setOrderStates(id) {
  ...
  dataOrderStates.forEach((dataOrderState) => {
    if (Number(dataOrderState.dataset.id) === Number(id))
      dataOrderState.dataset.orders = standardize(Tastey.getOrdersValue(id));
  });
}
...
function setLikeState(id) {
  ...
  dataLikeStates.forEach((dataLikeState) => {
    if (Number(dataLikeState.dataset.id) === Number(id))
      dataLikeState.dataset.like =
        Tastey.getLikeValue(id) ?? !dataLikeState.dataset.like;
  });
}
...
function toggleLoader(id, bool) {
  if (mealsQuery) {
    document
      .querySelector(`.tastey-meal[data-id="${id}"]`)
      ?.classList.toggle("loading", bool);
  }
  if (bagQuery) {
    document
      .querySelector(`.tastey-meal-order[data-id="${id}"]`)
      ?.classList.toggle("loading", bool);
  }
  if (miniBagQuery) {
    document
      .querySelector(`.mini-cart-tastey-meal-order[data-id="${id}"]`)
      ?.classList.toggle("loading", bool);
  }
  document
    .querySelector(`.footer-tastey-meal[data-id="${id}"]`)
    ?.classList.toggle("loading", bool);
}
...
function toggleDelLoader(id, bool) {
  if (mealsQuery) {
    document
      .querySelector(`.tastey-meal[data-id="${id}"]`)
      ?.classList.toggle("del-loading", bool);
  }
  if (bagQuery) {
    document
      .querySelector(`.tastey-meal-order[data-id="${id}"]`)
...
function toggleLikeLoader(id, bool) {
  if (mealsQuery) {
    document
      .querySelector(`.tastey-meal[data-id="${id}"]`)
      ?.classList.toggle("like-loading", bool);
  }
  if (bagQuery) {
    document
      .querySelector(`.tastey-meal-order[data-id="${id}"]`)
...
function editMeal(id) {
  ...
  if (bagQuery)
    document.querySelector(
      `.tastey-meal-order[data-id="${id}"] .cart-order-number`,
    ).textContent = orders;
  if (miniBagQuery)
    document.querySelector(
      `.mini-cart-tastey-meal-order[data-id="${id}"] .mini-cart-order-number`,
    ).textContent = orders;
}
...
function addMeal(id, meal) {
  ...
  if (bagQuery) {
    if (orders > 1) {
      document.querySelector(
        `.tastey-meal-order[data-id="${id}"] .cart-order-number`,
      ).textContent = standardize(orders);
    } else {
...
function removeMeal(id) {
  ...
  if (miniBagQuery) {
    const mcCurrentProductCountElement = document.querySelector(
      `.mini-cart-tastey-meal-order[data-id="${id}"] .mini-cart-order-number`,
    );
...
function setCheckoutState() {
  ...
  if (bagQuery) {
    ...
    totalDiscountElement.textContent =
      "-" + Tastey.totalDiscountPercentage + " %";
  ...
  if (miniBagQuery) {
    mcActualPriceElement.textContent = formatValue(
      currency,
      Tastey.actualAmount,
    );
...
function resetBagEventListeners() {
  if (bagQuery) {
    ...
    for (let i = 0; i < tasteyMealOrders.length; i++) {
      tasteyMealOrders[i].style.setProperty(
        "--card-position",
        Tastey.getOrdersPosition(tasteyMealOrders[i].dataset.id),
      );
      wishlistTogglers[i].onclick = () =>
        handleLikes(tasteyMealOrders[i]?.dataset.id);
      tasteyOrderImages[i].ondblclick = () =>
        handleLikes(tasteyMealOrders[i]?.dataset.id);
      deleteOrderBtns[i].onclick = () =>
        handleDeleteMeal(tasteyMealOrders[i]?.dataset.id);
      cartOrderNumbers[i].onkeydown = (e) =>
        handleContentEditableEnterKeyPress(e);
      cartOrderNumbers[i].onblur = (e) =>
        handleEditMealOrders(tasteyMealOrders[i].dataset.id, e);
      plusCartBtns[i].onclick = () =>
        handleAddMeal(tasteyMealOrders[i]?.dataset.id);
      minusCartBtns[i].onclick = () =>
        handleRemoveMeal(tasteyMealOrders[i]?.dataset.id);
    }
  }
  if (miniBagQuery) {
    ...
    for (let i = 0; i < mcTasteyMealOrders.length; i++) {
      mcTasteyMealOrders[i].style.setProperty(
        "--mini-card-position",
        Tastey.getOrdersPosition(mcTasteyMealOrders[i].dataset.id),
      );
      mcWishlistTogglers[i].onclick = () =>
        handleLikes(mcTasteyMealOrders[i]?.dataset.id);
      mcTasteyOrderImages[i].ondblclick = () =>
        handleLikes(mcTasteyMealOrders[i]?.dataset.id);
      mcDeleteOrderBtns[i].onclick = () =>
        handleDeleteMeal(mcTasteyMealOrders[i]?.dataset.id);
      mcCartOrderNumbers[i].onkeydown = (e) =>
        handleContentEditableEnterKeyPress(e);
      mcCartOrderNumbers[i].onblur = (e) =>
        handleEditMealOrders(mcTasteyMealOrders[i]?.dataset.id, e);
      mcPlusCartBtns[i].onclick = () =>
        handleAddMeal(mcTasteyMealOrders[i]?.dataset.id);
      mcMinusCartBtns[i].onclick = () =>
        handleRemoveMeal(mcTasteyMealOrders[i]?.dataset.id);
    }
  }
}

function getMealsIndex(id) {
  return Array.from(tasteyMeals).findIndex(
    (meal) => Number(meal.dataset.id) === Number(id),
  );
}

function getOrderIndex(id) {
  return Array.from(mcTasteyMealOrders || tasteyMealOrders).findIndex(
    (order) => Number(order.dataset.id) === Number(id),
  );
}
...
function setButtonState(id) {
  const orders = Tastey.getOrdersValue(id),
    ...
    tasteyMealOrder = document.querySelector(
      `.tastey-meal-order[data-id="${id}"]`,
    ),
    mcTasteyMealOrder = document.querySelector(
      `.mini-cart-tastey-meal-order[data-id="${id}"]`,
    ),
    footerAddToCartBtn = document.querySelector(
      `.footer-add-to-cart-button[data-id="${id}"]`,
    );
  if (mealsQuery) {
  ...
  try {
    ...
    if (await Tastey.deleteMeal(id)) {
      ...
      if (!getCardsQuery() && document.body.classList.contains("cart")) {
        ...
        if (bagQuery) removeCard(id, dir);
        if (miniBagQuery) removeMiniCard(id, dir);
    ...
    if (shouldCartClear) {
      ...
      if (await Tastey.clearCart()) {
        if (!getCardsQuery() && document.body.classList.contains("cart")) {
          ...
          if (bagQuery) removeAllCards();
          if (miniBagQuery) removeAllMiniCards();
          await new Promise((resolve) =>
            setTimeout(() => {
              clearCart();
              resolve();
...
async function handleLikes(id) {
  try {
    ...
    const like = mealsQuery
      ? !JSON.parse(tasteyMeal.dataset.like)
      : !JSON.parse(
          tasteyMealOrder?.dataset.like ?? mcTasteyMealOrder?.dataset.like,
        );
    ...
    if (await Tastey.handleLikes(id, like)) {
      if (mealsQuery) {
        ...
        tasteyMeal.querySelector(".heart-icon-wrapper").title = like
          ? "Tap to unlike"
          : "Tap to like!";
      }
      if (bagQuery && tasteyMealOrder) {
        tasteyMealOrder.dataset.like = like;
        tasteyMealOrder.querySelector(".wishlist-toggler").title = like
          ? `Remove ${label} from Wishlist`
          : `Add ${label} to Wishlist`;
      }
      if (miniBagQuery && mcTasteyMealOrder) {
