...
const archiveContentWrapper = document.querySelector(
...
function btnCheck(e) {
  if (e === "Still Tastey")
    return `<a href='menu.html' title='Open Menu' class='open-menu-btn'><span>OPEN OUR MENU</span></a>`;
  else return "";
}

document
  .querySelector(".open-menu-btn")
  .addEventListener("click", () => delete sessionStorage.open_cart);
...
function archiveUI() {
  if (window.innerHeight >= archives[0].offsetHeight * 1.5) {
    archives.forEach((archive) => {
      if (
        archive.getBoundingClientRect().top <
          window.innerHeight - remToPx(15) &&
        archive.getBoundingClientRect().bottom > remToPx(20)
      ) {
        setTimelinePosition(
          archive.querySelector(".archive-time-wrapper > h4"),
        );
        setTimeout(() => {
          if (
            archive.getBoundingClientRect().top <
    ...
    archives.forEach((archive) => {
      if (
        archive.getBoundingClientRect().top < window.innerHeight * 0.8 &&
        archive.getBoundingClientRect().bottom >
          remToPx(3) + window.innerHeight * 0.2
      ) {
        setTimelinePosition(
          archive.querySelector(".archive-time-wrapper > h4"),
        );
...
function setTimelinePosition(header) {
  ...
  archiveContentWrapper.style.setProperty(
    "--timeline-position",
    `${position}px`,
  );
}
...
function timelineLength() {
  ...
  const lastArchive = [...document.querySelectorAll(".archive-content")][
    Archives.length - 1
  ];
  let totalHeight =
    lastArchive
      .querySelector(".archive-time-wrapper > h4")
...
const firstValueCount = () => {
  ...
  fourthValueCount = () => {
    if (l < 50) {
      ...
      if (fourthValue.textContent === "50") fourthValue.textContent = "50 +";
    } else {
      clearInterval(r);
      r = null;
    }
...
let accrediationSectionObserver = new IntersectionObserver(
  (entries, observer) => {
...
accrediationSectionObserver.observe(
  document.querySelector("#accreditation-section-container"),
);
