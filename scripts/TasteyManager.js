...
function displayWelcomeNotification() {
  ...
  const welcomeMssg =
      !sessionStorage.tastey_friendship_log &&
      localStorage.tastey_friendship_log
        ? "Hello, Old Friend! Welcome back to Tastey"
        : "Hello, New Friend! Welcome to Tastey",
    title = "Tastey",
...
document
  .querySelector(".nav-link:nth-of-type(3)")
...
class TasteyManager {
  ...
  async editMeal(id, n = 1) {
    try {
      await mockQuery();
      const index = this.tasteyRecord.tasteyOrders.findIndex(
  ...
  async deleteMeal(id) {
    try {
      await mockQuery();
      const mealIndex = this.tasteyRecord.tasteyOrders.findIndex(
        (meal) => Number(meal.id) === Number(id),
      );
  ...
  async handleLikes(id, bool) {
    try {
      await mockQuery();
      const likeIndex = this.tasteyRecord.likes.findIndex(
        (meal) => Number(meal.id) === Number(id),
      );
      if (likeIndex === -1) {
        const meal = {};
  ...
  calculateCheckoutDetails(allMeals, curr) {
    try {
      ...
      this.ordersInTotal = this.tasteyRecord.tasteyOrders.reduce(
        (count, meal) => count + meal.orders,
        0,
      );
      this.actualAmount = 0;
      this.totalAmount = 0;
  ...
  getOrdersValue(id) {
    const meal = this.tasteyRecord.tasteyOrders.find(
      (meal) => Number(meal.id) === Number(id),
    );
  ...
  getOrdersPosition(id) {
    return (
      (this.tasteyRecord.tasteyOrders.findIndex(
        (meal) => Number(meal.id) === Number(id),
      ) ?? 0) + 1
    );
  }
  ...
  getLikeValue(id) {
    const like = this.tasteyRecord.likes.find(
      (meal) => Number(meal.id) === Number(id),
    );
    return like?.like;
  }
