...
function carousel() {
  ...
  const interval = 2500,
    ...
    currentSlideIndex = () => {
      return Math.round(
        tasteyMealsCarousel.scrollLeft / offsetWidth - nClonedSlides,
      );
    },
    scrollCarouselTo = (index) =>
      scrollContentTo(
        offsetWidth * (index + nClonedSlides),
        "smooth",
        tasteyMealsCarousel,
        "horizontal",
      ),
  ...
  tasteyMealsCarousel.addEventListener("scroll", (e) => {
    ...
    scrollTimer = setTimeout(() => {
      ...
      if (
        e.target.scrollLeft >
        offsetWidth * (nSlides - 1 + nClonedSlides + 1 / 2)
      )
        rewind();
    }, 100);
    // update the toggler
  ...
  function forward() {
    setTimeout(
      scrollContentTo,
      100,
      offsetWidth * (nSlides - 1 + nClonedSlides),
      "instant",
      tasteyMealsCarousel,
      "horizontal",
    );
  ...
  scrollCarouselTo(0);
  markToggler(0);
  if (carouselInView) setTimeout(play, 1000);
