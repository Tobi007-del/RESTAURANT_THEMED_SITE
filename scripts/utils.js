...
function standardize(C, manner = "use ease") {
  ...
  if (manner === "use ease") {
    ...
    const th = C / 1000,
      ...
      b = C / 1000000000,
      tr = C / 1000000000000;
    if (th < 1) return `${WN.format(C)}`;
    else if (th >= 1 && m < 1)
      if (th - Math.floor(th) !== 0) return `${FO.format(th)}K`;
      else return `${WN.format(th)}K`;
    else if (m >= 1 && b < 1)
      if (m - Math.floor(m) !== 0) return `${FO.format(m)}M`;
      else return `${WN.format(m)}M`;
    else if (b >= 1 && tr < 1)
      if (b - Math.floor(b) !== 0) return `${FO.format(b)}B`;
      else return `${WN.format(b)}B`;
    else if (tr >= 1)
      if (tr - Math.floor(tr) !== 0) return `${FO.format(tr)}T`;
      else return `${WN.format(tr)}T`;
  }
}
  ...
  function pan(element) {
    ...
    element.addEventListener("pointermove", (e) => {
      ...
      requestAnimationFrame(() => {
        let xPointerDelta =
            e.clientX - parseFloat(element.dataset.xPointerDownAt),
          yPointerDelta =
            e.clientY - parseFloat(element.dataset.yPointerDownAt),
